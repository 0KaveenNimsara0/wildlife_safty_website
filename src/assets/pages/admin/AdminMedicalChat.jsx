...
export default function AdminMedicalChat() {
  ...
  useEffect(() => {
    ...
    // If we have a paramOfficerId, we need to fetch their details and start/get conversation
    if (paramOfficerId) {
  ...
  const fetchOfficerDetails = async (id) => {
    try {
      setLoading(true);
      // We can get officer details from the admin/users/medical-officers endpoint or similar
      // For now, we'll try to find them in the conversations or fetch them
      ...
      if (data.success) {
        ...
        if (officer) {
          // Pre-select this officer
          // We need a conversation ID. The backend will generate it if we send a message, 
          // but for fetching, we might need to find an existing conversation or just show empty until first message
  ...
  const handleSelectOfficer = (officer, convId = null) => {
    // If officer comes from medical-officers list, we don't have convId yet
    // Generate ad-hoc conversationId for matching if needed, or let fetchMessages handle it if we have it
    setCurrentOfficer({
      ...officer,
    ...
    if (convId) {
      ...
      // If we don't have a convId, we might still be able to find it in conversations state
  ...
  const handleSendMessage = async (e) => {
    e.preventDefault();
    if (!newMessage.trim() || !currentOfficer) return;
    ...
    try {
      ...
      if (data.success) {
        ...
        // CRITICAL: If this was the first message, signal the polling system with the new ID
      ...
      <div className="flex-1 flex flex-col bg-white rounded-[32px] border border-slate-100 shadow-2xl shadow-slate-200/50 overflow-hidden relative">
        {currentOfficer ? (
          <>
                 ...
                 messages.map((msg, i) => {
                   ...
                   return (
                     <div key={msg._id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                        <div className={`max-w-[70%] group relative px-6 py-4 rounded-[20px] shadow-sm transform transition-all hover:scale-[1.02] ${
                          isMe 
                            ? 'bg-slate-900 text-white rounded-tr-none' 
                            : 'bg-white border border-slate-100 text-slate-700 rounded-tl-none'
                        }`}>
                           <p className="text-xs font-medium leading-relaxed font-sans">{msg.message}</p>
                           <div className={`flex items-center gap-2 mt-2 ${isMe ? 'justify-end' : 'justify-start'}`}>
                              <span className={`text-[8px] font-bold uppercase tracking-widest ${isMe ? 'text-slate-400' : 'text-slate-400'}`}>
                                 {formatTime(msg.createdAt)}
                              </span>
                              {isMe && (
                                 msg.isRead ? <CheckCheck size={12} className="text-emerald-400" /> : <Check size={12} className="text-slate-500" />
                              )}
                           </div>
                        </div>
                     </div>
            ...
            {/* Input */}
            <div className="p-8 bg-white/80 backdrop-blur-sm border-t border-slate-50">
               <form onSubmit={handleSendMessage} className="relative flex items-center gap-4">
                  <div className="flex-1 relative group">
                     ...
                     <input 
                       ...
                       className="w-full pl-16 pr-6 py-5 bg-slate-50 border-transparent rounded-[24px] focus:bg-white focus:border-emerald-500 focus:outline-none text-xs font-black uppercase tracking-widest transition-all shadow-inner focus:ring-4 focus:ring-emerald-500/5 placeholder:text-slate-300" 
                     />
                  </div>
                  <button 
                    type="submit"
                    disabled={!newMessage.trim()}
                    className="p-5 bg-emerald-600 text-white rounded-2xl shadow-xl shadow-emerald-500/20 hover:bg-emerald-500 transition-all hover:-translate-y-1 active:scale-95 disabled:opacity-50 disabled:translate-y-0 group"
                  >
