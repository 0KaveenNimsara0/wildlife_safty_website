...
export default function AdminArticleManagement() {
  ...
  const fetchArticles = async () => {
    try {
      ...
      const endpoint = statusFilter === 'all'
        ? `${BASE_URL}/admin/articles?page=${currentPage}&limit=20`
        : `${BASE_URL}/admin/articles/status/${statusFilter}?page=${currentPage}&limit=20`;
  ...
  const handleDeleteArticle = async (articleId) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      const response = await fetch(`${BASE_URL}/admin/articles/${articleId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const handlePublishArticle = async (articleId) => {
    try {
      const token = localStorage.getItem('adminToken');
      const response = await fetch(`${BASE_URL}/admin/articles/${articleId}/publish`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`
        }
