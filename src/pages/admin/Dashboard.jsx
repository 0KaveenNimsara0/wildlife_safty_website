...
export default function AdminDashboard() {
  ...
  const fetchStats = async () => {
    try {
      const token = localStorage.getItem('adminToken');
      const response = await fetch(`${BASE_URL}/admin/users/stats/overview`, {
        headers: {
    ...
    <div className="space-y-10">
      <div className="relative overflow-hidden bg-slate-900 rounded-[2.5rem] p-10 text-white shadow-2xl">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
          <div>
             ...
             <h1 className="text-4xl text-white tracking-tight uppercase">Admin <span className="text-emerald-400">Dashboard</span></h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          const colorClasses = {
            emerald: { bg: 'bg-emerald-500/10', text: 'text-emerald-600', trend: 'bg-emerald-50 text-emerald-600' },
            blue: { bg: 'bg-blue-500/10', text: 'text-blue-600', trend: 'bg-blue-50 text-blue-600' },
            amber: { bg: 'bg-amber-500/10', text: 'text-amber-600', trend: 'bg-amber-50 text-amber-600' },
            rose: { bg: 'bg-rose-500/10', text: 'text-rose-600', trend: 'bg-rose-50 text-rose-600' }
          };
          ...
          return (
            <div key={idx} className="card-premium group relative overflow-hidden p-8 bg-white border-slate-100">
               ...
               <div className="flex items-start justify-between mb-6">
                  <div className={`p-4 rounded-2xl ${cls.bg} ${cls.text}`}>
                     <card.icon size={20} />
                  </div>
                  <span className={`text-[10px] font-black px-2 py-1 rounded-lg ${cls.trend} tracking-widest`}>
                     {card.trend}
                  </span>
               </div>
               <div>
