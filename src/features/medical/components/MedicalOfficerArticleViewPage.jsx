...
export default function MedicalOfficerArticleViewPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen py-20 flex flex-col items-center justify-center space-y-4 opacity-30">
        ...
        <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">Loading Content...</p>
      </div>
    );
  }
  ...
  if (error || !article) {
    return (
      <div className="min-h-screen py-20 px-6 flex flex-col items-center justify-center text-center">
        ...
        <button 
          onClick={() => navigate('/medical-officer/articles')}
          className="px-8 py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600 transition-all flex items-center gap-3"
        >
          <ArrowLeft size={16} />
          Back to Hub
        </button>
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-12 animate-fade-in">
      ...
      >
        <div className="p-2.5 rounded-xl bg-slate-50 group-hover:bg-indigo-50 transition-all">
          <ArrowLeft size={18} />
        </div>
        <span className="text-[10px] font-black uppercase tracking-[0.2em]">Back to Article Hub</span>
      </button>

      {/* Article Header Section */}
      <div className="space-y-8 mb-16">
        <div className="flex flex-wrap items-center gap-3">
          <span className="px-4 py-1.5 bg-indigo-500 text-white rounded-full text-[9px] font-black uppercase tracking-widest">
            {article.category || 'Clinical Research'}
          </span>
          <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border ${
            article.status === 'published' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-amber-50 text-amber-600 border-amber-100'
          }`}>
        ...
        <div className="flex flex-wrap items-center gap-8 pt-4 border-t border-slate-100">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-indigo-600 text-white flex items-center justify-center font-black text-sm shadow-lg shadow-indigo-600/20">
              {article.author?.name?.charAt(0).toUpperCase()}
            </div>
            <div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Primary Author</p>
          ...
          <div className="flex items-center gap-3 text-slate-400">
            <Clock size={18} />
            <div>
              <p className="text-[10px] font-black uppercase tracking-widest mb-0.5 text-slate-400">Published Date</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
        <div className="lg:col-span-8 flex flex-col">
          <div 
            className="prose prose-slate prose-lg max-w-none 
              ...
              pt-4 border-t-4 border-indigo-600 w-24 mb-12"
          >
            {/* Full Content Rendering */}
            <div className="text-xl font-medium text-slate-700 leading-loose space-y-8" 
        ...
        <div className="lg:col-span-4 space-y-12">
          <div className="p-8 rounded-[40px] bg-slate-50 border border-slate-100">
            <h4 className="text-[11px] font-black uppercase tracking-widest text-indigo-600 mb-6">Medical Oversight</h4>
            <div className="space-y-6">
              <div className="flex items-center gap-4 text-slate-600">
                 ...
                 <div className="text-[10px] font-bold uppercase tracking-widest">
                    Study Verified by Professional Node
                 </div>
              </div>
              <div className="flex items-center gap-4 text-slate-600">
                 ...
                 <div className="text-[10px] font-bold uppercase tracking-widest">
                    Available for Peer Discussion
                 </div>
              </div>
            </div>
          </div>

          <div className="p-10 rounded-[48px] bg-indigo-600 text-white shadow-2xl shadow-indigo-600/20">
             <h4 className="text-[10px] font-black uppercase tracking-widest mb-4 opacity-60">Legal Disclaimer</h4>
             <p className="text-xs font-medium leading-relaxed opacity-90 italic">
                This content is for clinical and identification support. Always verify field data with physical evidence before proceeding with medical response protocols.
             </p>
          </div>
        </div>
      </div>
